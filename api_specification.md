...
7. [顧客マスタAPI](#顧客マスタapi)
8. [営業マスタAPI](#営業マスタapi)
9. [所属部署マスタAPI](#所属部署マスタapi)
10. [エラーコード一覧](#エラーコード一覧)
...
| password   | 必須、10文字以上、大文字・小文字・数字・特殊文字を含む |
...
```json
{
  ...
  "data": {
    "departments": [
      ...
      {
        "department_id": "3",
        "department_name": "営業3課",
        "display_order": 3
      },
      {
        "department_id": "4",
        "department_name": "営業4課",
        "display_order": 4
      }
    ]
  }
...
- 所属部署はプルダウン選択肢として使用されるため、全件取得（ページネーションなし）
- `display_order` の昇順で並べて返す
- 認証が必要
